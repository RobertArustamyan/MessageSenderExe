...
class CustomParseAndMessage(ParseAndMessage):
    def __init__(self, driver, config, pages, progress_callback=None):
        super().__init__(driver, config, pages)
    ...
    def send_messages(self, messages, send_to_all=True, test_mode=True):
        ...
        if self.progress_callback:
            self.progress_callback("Initializing message sending process...")
        ...
        try:
            if self.progress_callback:
                self.progress_callback("Parsing offers from pages...")
            all_offers = self._get_all_offers()
            logger.info(f"Found {len(all_offers)} total offers to process")
            if self.progress_callback:
                self.progress_callback(f"Found {len(all_offers)} offers to process")
        except Exception as e:
            logger.error(f"Error getting all offers: {e}")
            if self.progress_callback:
                self.progress_callback("Error getting offers from pages")
            return
        ...
        for i, offer in enumerate(all_offers, 1):
            if self.progress_callback:
                self.progress_callback(
                    f"Processing offer {i}/{len(all_offers)} | Success: {successful_messages} | Skipped: {skipped_messages} | Failed: {failed_messages}",
                    f"Processing {i}/{len(all_offers)}"
                )
            ...
            try:
                ...
                if not test_mode:
                    send_button.click()
                logger.info(f"Message would be sent to {phone_number} (test_mode={test_mode})")
        ...
        # Final summary
        summary_message = (
            f"Process completed! Total: {len(all_offers)} | "
            f"Success: {successful_messages} | "
            f"Skipped: {skipped_messages} | "
            f"Failed: {failed_messages}"
        )

        if self.progress_callback:
            self.progress_callback(summary_message, "Process completed")
